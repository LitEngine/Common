...
#if defined(ENGINE_EXIT_HARD) && ENGINE_EXIT_HARD
#define ENGINE_EXIT exit(1);
#else
#define ENGINE_EXIT
#endif

#define ENGINE_ASSERT(exp, msg) \
{ if (!(exp)) { std::cerr << "[ENGINE ASSERT]" << msg << ":" << __FILE__ << ":" << __LINE__ << std::endl; ENGINE_EXIT }}

#define ENSURE_THAT(constraint) \
{ if (!(constraint)) { std::cerr << "[Constraint violated] \"" << #constraint << "\" is false" << std::endl << \
"Location: " << __FILE__ << ":" << __LINE__ << std::endl << std::endl; ENGINE_EXIT }}
